...
void Sensor_DoConversion(void) {
	...
	*/
	...
	for (int i = 0; i < 4; i++) { // Get 4 TC channels
		tcpresent[i] = 0;//OneWire_IsTCPresent(i);
		if (tcpresent[i]) {
			tctemp[i] = 0;//OneWire_GetTCReading(i);
			tccj[i] = 0;//OneWire_GetTCColdReading(i);
			if (i > 1) {
	...
	if (tcpresent[0] && tcpresent[1]) {
		...
		coldjunction = 128.0;//OneWire_GetTempSensorReading();
