...
static int32_t SPI_TC_Work( void ) {
	for (int i = 0; i < numspidevices; i++) {
		...
		if (SC18IS602B_SPI_Xfer(&xfer) == 0) {
			...
			//printf("(%d 0x%04x 0x%04x)", i, (uint16_t)spidevreadout[i], (uint16_t)spiextrareadout[i]);
		}
	}
	return TICKS_MS(100);
}
...
uint32_t SPI_TC_Init(I2C_HandleTypeDef* i2c) {
	...
	if (SC18IS602B_Init(i2c, SPICLK_1843KHZ, SPIMODE_0, SPIORDER_MSBFIRST) >= 0) {
		printf("Probing for MAX31855 devices...");
		...
		for (int i = 0; i < MAX_SPI_DEVICES; i++) {
			if ((spidevreadout[i] == -1 && spiextrareadout[i] == -1) ||
				...
				numspidevices = i + 1;
				printf("%f %f", SPI_GetTCReading(i), SPI_GetTCColdReading(i));
			}
		}
