...
#define F4KEY_PORTBIT (1 << 4) // No F4 on T-962A+...
...
//	return ~FIO0PIN & (F1KEY_PORTBIT | F2KEY_PORTBIT | F3KEY_PORTBIT | F4KEY_PORTBIT | S_KEY_PORTBIT);
	return ~KEY_F1_GPIO_Port->IDR & (KEY_F1_Pin | KEY_F2_Pin | KEY_F3_Pin | KEY_S_Pin);
}
...
static int32_t Keypad_Work(void) {
	...
	if (changed) {
		if (changed & KEY_F1_Pin) keypadstate |= KEY_F1;
		if (changed & KEY_F2_Pin) keypadstate |= KEY_F2;
		if (changed & KEY_F3_Pin) keypadstate |= KEY_F3;
		//if (changed & F4KEY_PORTBIT) keypadstate |= KEY_F4;
		if (changed & KEY_S_Pin) keypadstate |= KEY_S;
	}
