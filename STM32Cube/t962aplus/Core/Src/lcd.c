...
#define UNTIL_BUSY_IS_CLEAR while (GPIOC->IDR & 0x80); // Wait for busy to clear
...
static void LCD_WriteCmd(uint32_t cmdbyte) {
	...
	GPIOC->CRL = 0x44444444; //FIO1DIR = 0x000000; // Data pins are now inputs
	LCD_RS_GPIO_Port->BRR = LCD_RS_Pin; LCD_CS1_GPIO_Port->BRR = LCD_CS1_Pin; //FIO0CLR = (1 << 22) | (1 << 13); // RS low, also make sure other CS is low
	LCD_CS2_GPIO_Port->BSRR = LCD_CS2_Pin; //FIO0SET = (1 << 12); // One CS at a time
	LCD_RW_GPIO_Port->BSRR = LCD_RW_Pin; //FIO0SET = (1 << 19); // RW must go high before E does
	LCD_E_GPIO_Port->BSRR = LCD_E_Pin; //FIO0SET = (1 << 18); // E high for read
	GPIOC->IDR; //FIO1PIN; // Need 320ns of timing margin here
	GPIOC->IDR; //FIO1PIN;
	GPIOC->IDR; //FIO1PIN;
	GPIOC->IDR; //FIO1PIN;
	GPIOC->IDR; //FIO1PIN;
	GPIOC->IDR; //FIO1PIN;
	UNTIL_BUSY_IS_CLEAR;
	LCD_CS2_GPIO_Port->BRR = LCD_CS2_Pin; //FIO0CLR = (1 << 12); // Swap CS
	LCD_E_GPIO_Port->BRR = LCD_E_Pin; //FIO0CLR = (1 << 18); // E low again
	GPIOC->IDR; //FIO1PIN;
	GPIOC->IDR; //FIO1PIN;
	GPIOC->IDR; //FIO1PIN;
	GPIOC->IDR; //FIO1PIN;
	...
	GPIOC->IDR; //FIO1PIN;
	GPIOC->IDR; //FIO1PIN;
	LCD_CS1_GPIO_Port->BSRR = LCD_CS1_Pin; //FIO0SET = (1 << 13); // One CS at a time
	LCD_E_GPIO_Port->BSRR = LCD_E_Pin; //FIO0SET = (1 << 18); // E high for read
	GPIOC->IDR; //FIO1PIN; // Need 320ns of timing margin here
	GPIOC->IDR; //FIO1PIN;
	GPIOC->IDR; //FIO1PIN;
	...
	GPIOC->IDR; //FIO1PIN;
	UNTIL_BUSY_IS_CLEAR;
	LCD_E_GPIO_Port->BRR = LCD_E_Pin; LCD_RW_GPIO_Port->BRR = LCD_RW_Pin; //FIO0CLR = (1 << 19) | (1 << 18); // RW + E low again
	GPIOC->CRL = 0x11111111; //FIO1DIR = 0xff0000; // Data pins output again

	LCD_CS2_GPIO_Port->BSRR = LCD_CS2_Pin; //FIO0SET = (1 << 12) | (1 << 13); // Both CS active (one already activated above, doesn't matter)
	GPIOC->BSRR = 0x00ff0000 | cmdbyte; //FIO1PIN = cmdbyte << 16; // Cmd on pins
	GPIOC->IDR; //FIO1PIN; // Need 320ns of timing margin here
	GPIOC->IDR; //FIO1PIN;
	GPIOC->IDR; //FIO1PIN;
	GPIOC->IDR; //FIO1PIN;
	GPIOC->IDR; //FIO1PIN;
	GPIOC->IDR; //FIO1PIN;
	LCD_E_GPIO_Port->BSRR = LCD_E_Pin; //FIO0SET = (1 << 18); // E high
	GPIOC->IDR; //FIO1PIN;
	GPIOC->IDR; //FIO1PIN;
	GPIOC->IDR; //FIO1PIN;
	GPIOC->IDR; //FIO1PIN;
	...
	GPIOC->IDR; //FIO1PIN;
	GPIOC->IDR; //FIO1PIN;
	LCD_E_GPIO_Port->BRR = LCD_E_Pin; //FIO0CLR = (1 << 18); // E low
	GPIOC->IDR; //FIO1PIN;
	GPIOC->IDR; //FIO1PIN;
	GPIOC->IDR; //FIO1PIN;
	GPIOC->IDR; //FIO1PIN;
	GPIOC->IDR; //FIO1PIN;
	GPIOC->IDR; //FIO1PIN;
	LCD_RS_GPIO_Port->BSRR = LCD_RS_Pin; //FIO0SET = (1 << 22); // RS high
}
...
static inline void LCD_WriteData(uint32_t databyte, uint8_t chipnum) {
	...
	LCD_CS1_GPIO_Port->BSRR = LCD_CS1_Pin << (chipnum ? 16 : 0);
	LCD_CS2_GPIO_Port->BSRR = LCD_CS2_Pin << (chipnum ? 0 : 16);
	GPIOC->CRL = 0x44444444; //FIO1DIR = 0x000000; // Data pins are now inputs
	LCD_RS_GPIO_Port->BRR = LCD_RS_Pin; //FIO0CLR = (1 << 22); // RS low
	LCD_RW_GPIO_Port->BSRR = LCD_RW_Pin; //FIO0SET = (1 << 19); // RW must go high before E does
	LCD_E_GPIO_Port->BSRR = LCD_E_Pin; //FIO0SET = (1 << 18); // E high for read
	GPIOC->IDR; //FIO1PIN; // Need 320ns of timing margin here
	GPIOC->IDR; //FIO1PIN;
	...
	GPIOC->IDR; //FIO1PIN;
	UNTIL_BUSY_IS_CLEAR;
	LCD_E_GPIO_Port->BRR = LCD_E_Pin; LCD_RW_GPIO_Port->BRR = LCD_RW_Pin; //FIO0CLR = (1 << 18) | (1 << 19); // E and RW low
	LCD_RS_GPIO_Port->BSRR = LCD_RS_Pin; //FIO0SET = (1 << 22); // RS high
	GPIOC->CRL = 0x11111111; //FIO1DIR = 0xff0000; // Data pins output again
	GPIOC->BSRR = 0x00ff0000 | databyte; //FIO1PIN = databyte << 16; // Data on pins
	GPIOC->IDR; //FIO1PIN; // Need ~200ns of timing margin here
	GPIOC->IDR; //FIO1PIN;
	GPIOC->IDR; //FIO1PIN;
	GPIOC->IDR; //FIO1PIN;
	GPIOC->IDR; //FIO1PIN;
...
void LCD_Init(void) {
	GPIOC->CRL = 0x11111111; //FIO1DIR = 0xff0000; // Data pins output
	LCD_WriteCmd(LCD_ON);
