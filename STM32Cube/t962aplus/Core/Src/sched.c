...
void Sched_Init(TIM_HandleTypeDef* timer) {
	s_timer = timer;
...
//	return T0TC;
	// Stupid STM32 with no 32-bit counter
	uint32_t tmp = s_timer->Instance->CNT;
	uint32_t tmp2 = TIM3->CNT;
	uint32_t tmp3 = s_timer->Instance->CNT;
	uint32_t tmp4 = TIM3->CNT;
